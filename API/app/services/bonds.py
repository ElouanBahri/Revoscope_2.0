...
Ported from the original Streamlit revoscope's revoscope/bonds.py, with
...
_BROWSER_HEADERS = {"User-Agent": "Mozilla/5.0 (compatible; revoscope/2.0)"}
...
def get_current_treasury_yield_curve() -> dict[float, float] | None:
    ...
    once a new one is out, without hammering the endpoint on every request.
    """
    try:
        response = requests.get(
...
def estimate_bond_economics(pos: Position) -> BondEconomics:
    """Best-effort bond terms derived only from the trade history, for bonds
    the Treasury API doesn't cover (corporate bonds, non-US governments).
